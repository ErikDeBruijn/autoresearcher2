...
def test_worker_claims_and_executes(store):
    _add_todo(store)
    execute_called = []
    ...
    assert store.count_proposals("todo") == 0


def test_worker_returns_none_when_empty(store):
    worker = Worker(store, execute_fn=lambda p: {})
    result = worker.tick()
    assert result is None


def test_worker_handles_execution_failure(store):
    _add_todo(store)

    def failing_execute(proposal):
        raise RuntimeError("GPU out of memory")

    worker = Worker(store, execute_fn=failing_execute)
    result = worker.tick()
...
def test_worker_executes_highest_priority_first(store):
    _add_todo(store, intent="low priority", rank=5)
    _add_todo(store, intent="high priority", rank=1)
    ...
    worker = Worker(store, execute_fn=track_execute)
    worker.tick()

    assert executed[0] == "high priority"


def test_worker_run_with_max_ticks(store):
    _add_todo(store, intent="task 1")
    _add_todo(store, intent="task 2")
    ...
    assert store.count_proposals("done") == 2
    assert store.count_proposals("todo") == 0
